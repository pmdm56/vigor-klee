...
class KeysFromKleeExpr : public klee::ExprVisitor::ExprVisitor {
private:
...
public:
  KeysFromKleeExpr(const BMv2SimpleSwitchgRPC_Generator &_generator)
      : ExprVisitor(false), generator(_generator) {}

  const std::vector<std::string> &get_keys() { return keys; }
