...
public:
  PacketGetUnreadLength()
    : Module(ModuleType::x86_PacketGetUnreadLength, Target::x86, "PacketGetUnreadLength") {}
...
private:
  ...
  BDD::BDDVisitor::Action visitCall(const BDD::Call* node) override {
    ...
    if (call.function_name == "packet_get_unread_length") {
      auto ep_node  = ExecutionPlanNode::build(CREATE_SHARED_MODULE(PacketGetUnreadLength), node);
