...
class Forward : public __Module {
public:
  Forward() : __Module(ModuleType::x86_Forward, Target::x86, "Forward") {}
...
private:
  ...
  BDD::BDDVisitor::Action visitReturnProcess(const BDD::ReturnProcess* node) override {
    if (node->get_return_operation() == BDD::ReturnProcess::Operation::FWD) {
      auto ep_node  = ExecutionPlanNode::build(CREATE_SHARED_MODULE(Forward), node);
