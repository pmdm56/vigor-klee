...
private:
  bool is_valid_ipv4(const BDD::Node *ethernet_node, klee::ref<klee::Expr> len, const std::vector<klee::ConstraintManager> & constraints){
    ...
    auto eth_type_expr = BDD::solver_toolbox.exprBuilder->Extract(ethernet_chunk, 12*8, 2*8);
    auto eth_type_ipv4 = BDD::solver_toolbox.exprBuilder->Constant(UINT_16_SWAP_ENDIANNESS(0x0800), 2*8);
    ...
    for( auto constraint : constraints ){
      if(!BDD::solver_toolbox.is_expr_always_true(constraint, eq, symbol_replacer)){
        return false;
      }
    }
  ...
  BDD::BDDVisitor::Action visitCall(const BDD::Call *node) override {
    ...
    is_valid_ipv4(all_prev_packet_borrow_next_chunk[0], _length, node->get_constraints());
